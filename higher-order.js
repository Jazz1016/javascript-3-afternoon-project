...
let evenNumbers = mixedNumbers.filter((num, i, arr) => {
  if (num % 2 === 0) {
    return true;
  } else return false;
});
...
//Code Here
let totalPopulation = populations.reduce((sum, pop, i, arr) => {
  return sum + pop;
}, 0);
...
//Code Here
let myStrongest = monstersInYourPocket.filter((monstObj, i, arr) => {
  if (monstObj.CP > 200) {
    return true;
  } else {
    return false;
...
let orderTotals = orders.map((order, i, arr) => {
...
let bobsTotal = purchases.reduce((sum, purchase, i, arr) => {
